...
describe("/api/psychologists", () => {
  it("should return bad request if pass wrong params in query", async () => {
    let result;
    await axios
      .get(`${config.nextAuth.url}/api/psychologists?wrong=0`)
      .catch((e) => {
        result = e;
      });
  ...
  it("should return empty array if no params", async () => {
    const result = await axios.get(`${config.nextAuth.url}/api/psychologists`);
  ...
  it("should return some results if coordinates passed", async () => {
    const result = await axios.get(
      `${config.nextAuth.url}/api/psychologists?longitude=1.4328&latitude=43.6007`
    );
